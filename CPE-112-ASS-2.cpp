...
int main() /* dili ni siya pwd ma wala kay mao niy pinaka main kung wala ni dili mo run ang program  */
 {
	   ...
	   /* kani sya kay mga variable nga gi declare 
	      para gamiton sa code 
              nga pagahimuon  */
	cpe1b guwapo;
	long num,i;
	string str;
    ...
    cout << "The size of str is " << str.length() << " bytes.\n";    /* gi print ang kung pilay size sa gi enter */
		...
		if(str.length()%2==0)  /* gi analyze nya kung odd bha or even ang gi input  */
	{
		cout<<str.length()<<" is EVEN"<<endl;  /* then base sa condition sa babaw kung mo equal sya ug zero EVEN sya then gi tawag si str. length para ma apil ug print */
	}
	else    /* nag gamit ug else para naay laing option ang if statement kung dili mo equal ug zero  */
	{
		cout<<str.length()<<" is ODD"<<endl;    /* kani siya kay ga base gihapon sa condition sa babaw kay na sulod na mn sya sa else so kung dili mo equal to zero kay odd sya */
	}
